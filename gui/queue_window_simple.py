...
class SimpleQueueManagerWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.insert_queue_btn.clicked.connect(self.show_insert_dialog)
        control_layout.addWidget(self.insert_queue_btn)
        
        self.manual_add_btn = QPushButton("手动添加")
        self.manual_add_btn.clicked.connect(self.show_manual_add_dialog)
        control_layout.addWidget(self.manual_add_btn)
    ...
    def show_insert_dialog(self):
        """显示插队对话框"""
        available_items = self.queue_manager.get_available_items()
        if not available_items:
            QMessageBox.warning(self, "提示", "没有可用的名单项目")
            return
            
        dialog = InsertQueueDialog(available_items, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            selected_item = dialog.get_selected_item()
            if selected_item:
                # 检查用户是否已在队列中
                if selected_item.name in self.queue_manager.user_queued:
                    QMessageBox.warning(self, "插队失败", f"{selected_item.name} 已在排队队列中")
                    return
                
                success = self.queue_manager.insert_queue(selected_item)
                if success:
                    self.refresh_ui()  # 直接刷新UI
                    self.log_widget.log_queue_success(selected_item.name, "插队", 2)
                    QMessageBox.information(self, "插队成功", f"{selected_item.name} 已插队到队列最前面")
                else:
                    self.log_widget.log_queue_failed(selected_item.name, "次数不足且无其他相同名字项目")
                    QMessageBox.warning(self, "插队失败", 
                                      f"{selected_item.name} 次数不足且无其他相同名字项目")
    ...
    def show_manual_add_dialog(self):
        ...
        dialog = ManualAddQueueDialog(available_items, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            selected_item = dialog.get_selected_item()
            add_type = dialog.add_type
            
            if selected_item:
                if add_type == "queue":
                    # 添加到排队
                    if selected_item.name in self.queue_manager.user_queued:
                        QMessageBox.warning(self, "添加失败", f"{selected_item.name} 已在排队队列中")
                        return
                    
                    success = self.queue_manager.add_queue(selected_item.name)
                    if success:
                        self.refresh_ui()
                        self.log_widget.log_queue_success(selected_item.name, "手动添加排队", 1)
                        QMessageBox.information(self, "添加成功", f"{selected_item.name} 已添加到排队队列")
                    else:
                        self.log_widget.log_queue_failed(selected_item.name, "添加失败：次数不足或其他错误")
                        QMessageBox.warning(self, "添加失败", f"{selected_item.name} 添加失败，请检查次数是否足够")
                        
                elif add_type == "boarding":
                    # 添加到上车
                    if selected_item.name in self.queue_manager.user_boarded:
                        QMessageBox.warning(self, "添加失败", f"{selected_item.name} 已在上车队列中")
                        return
                      # 手动添加上车，传递is_manual=True，不受功能开关限制
                    success = self.queue_manager.process_boarding_request(selected_item.name, is_manual=True)
                    if success:
                        self.refresh_ui()
                        self.log_widget.log_queue_success(selected_item.name, "手动添加上车", 1)
                        QMessageBox.information(self, "添加成功", f"{selected_item.name} 已添加到上车队列")
                    else:                        # 只有次数不足或用户已在队列中才会失败
                        self.log_widget.log_queue_failed(selected_item.name, "添加失败：次数不足或用户已在队列中")
                        QMessageBox.warning(self, "添加失败", f"{selected_item.name} 添加失败，请检查次数是否足够")
                        
    def requeue(self):
        """再次排队"""
        reply = QMessageBox.question(
            self, "确认", "确定要清空排队队列和上车队列重新开始吗？",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # 直接清空排队队列和上车队列数据
              # 重置所有名单项目的in_queue和in_boarding标志
