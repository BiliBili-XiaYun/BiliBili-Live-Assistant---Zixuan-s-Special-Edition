...
class QueueManager:
    ...
    def __init__(self, name_list_file: str = None,
        ...
        if self.name_list_file and os.path.exists(self.name_list_file):
            self.load_name_list()
            # load_name_list()内部已经有完整的日志输出，这里只需要记录初始化完成
            self.queue_logger.info("QueueManager初始化完成", f"共 {len(self.name_list)} 个项目")
    ...
    def _load_name_list_silent(self) -> bool:
        """
        静默加载名单文件（不输出详细日志，避免重复）
        ...
        try:
            ...
            self.queue_logger.debug("静默加载名单失败", str(e))
            return False
        ...
        try:
            ...
            if new_path and new_path.strip():
                ...
                if new_abs_path != self.name_list_file:
                    ...
                    self.name_list_file = new_abs_path
                  # 重新加载名单文件（静默加载，避免重复日志）
                success = self._load_name_list_silent()
                if success:
    ...
    def is_queue_active(self) -> bool:
        """检查排队是否处于活跃状态"""
        return self.queue_started
    
    def is_cutline_active(self) -> bool:
        """检查插队是否处于活跃状态"""
        return self.cutline_started
    ...
    def load_data(self) -> bool:
        ...
        try:
            ...
            if success:
                self.queue_logger.info("数据加载完成", f"共加载 {len(self.name_list)} 个项目")
            return success
        except Exception as e:
            self.queue_logger.error("加载数据失败", str(e))
            return False
    ...
    def add_to_queue(self, username: str) -> bool:
        """添加用户到排队队列"""
        return self.process_queue_request(username)
    ...
    def clear_queue(self) -> bool:
        ...
        try:
            ...
            self.queue_logger.error("清空队列失败", str(e))
            return False
    
    def get_queue_list(self) -> List[QueueItem]:
        """
        获取排队列表
        
        Returns:
            List[QueueItem]: 当前排队列表
        """
        return self.queue_list.copy()
    
    def get_name_list(self) -> List[QueueItem]:
        """
        获取名单列表
        
        Returns:
            List[QueueItem]: 当前名单列表
        """
        return self.name_list.copy()
    
    def load_data(self):
        """加载数据 - 兼容性方法"""
        # 数据已经在初始化时加载了，这里只是为了兼容性
        pass